...
Scenario Outline: Adding description (like his hobbies, additional expertise or anything else in max. 600, alphanumeric or special characters) with valid inputs
	...
	Then a pop up message 'Description has been saved successfully' will appear
...
Scenario: Saving description with missing inputs
	...
	And clicks on “Save” button
	Then an error pop up message 'Please, a description is required' will appear
...
Scenario Outline: Saving Languages with missing inputs
	...
	And he enters '<Language>' and '<Level>'
	And clicks on “Add” button
	Then an error popup message '<Message>' will appear
...
Scenario Outline: Adding Languages with duplicate data
	Given Seller is on Profile Page
	And no languages has been added
	When he clicks on “Add New” button under "Languages" tab
...
Scenario Outline: Saving Certifications with missing inputs
	...
	And he enters '<Certificate>', '<From>', '<Year>' in Certifications
	And clicks on “Add” button
	Then an error popup message 'Please enter Certification Name, Certification From and Certification Year' will appear
...
Scenario Outline: Updating Sellers name
	Given Seller is on Profile Page
	And First and Last name is already added to the Profile Page
	When he click on dropdown arrow, just before his name
	And updates his '<First Name>' and '<Last Name>'
	And clicks on "Save" button
	Then he must be able to see '<First Name>' and '<Last Name>' updated on the Profile Page
